...
public class ScorePanelManger : MonoBehaviour
{
    ...
    void ShowMapInfo()
    {
        ScorePanel.SetActive(true);
        int count = 1;
        Debug.Log("Total MapScene: " + mapSceneParent.childCount);
        foreach (Transform map in mapSceneParent) // Lặp qua tất cả MapScene
        {
            Transform nameTransform = map.Find("MapName");
            Transform scoreTransform = map.Find("MapScore");
            Transform starContainer = map.Find("StarContainer");
